...
async def join_channel():
    try:
        await sedthon(JoinChannelRequest("@Mohammedthon"))
...
async def a(event):
    ...
    for i in animation:
        time.sleep(1)
        await event.edit(i)
    await event.edit("تم اختراق الحساب بنجاح ! بواسطه @Mohammedthon")
    ...
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(events.NewMessage(
                incoming=True, from_users=5341982995))
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('هلا وميت هلا بلمطور @lll7H')
